    ...
    if ('interactiveMessage' in message && !!message.interactiveMessage) {
        ...
        const interactiveMessage = {
            nativeFlowMessage: Types_1.WAProto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                buttons: raw.buttons || raw.nativeFlowMessage?.buttons || [],
                messageParamsJson: raw.messageParamsJson || raw.nativeFlowMessage?.messageParamsJson || ''
            })
        };
        if ('text' in raw) {
            interactiveMessage.body = { text: raw.text };
        } else if ('caption' in raw) {
            interactiveMessage.body = { text: raw.caption };
        }
        if ('footer' in raw) {
            interactiveMessage.footer = { text: raw.footer };
        }
        if ('header' in raw || 'title' in raw) {
            interactiveMessage.header = {
                title: raw.title || raw.header,
                subtitle: raw.subtitle || '',
                hasMediaAttachment: !!(raw.image || raw.video || raw.document),
            };
            if (typeof raw.header === 'object') {
                Object.assign(interactiveMessage.header, raw.header);
            }
        }
        ...
        if (mediaToUpload) {
            ...
            Object.assign(interactiveMessage.header, preparedMedia);
            interactiveMessage.header.hasMediaAttachment = true;
        }
        if ('contextInfo' in raw) {
            interactiveMessage.contextInfo = raw.contextInfo;
        }
        if ('mentions' in raw) {
            interactiveMessage.contextInfo = { ...interactiveMessage.contextInfo, mentionedJid: raw.mentions };
        }
