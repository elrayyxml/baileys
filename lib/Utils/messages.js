...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
       ...
       pollResultSnapshotMessage.contextInfo = {
        ...
        const stickerData = {}
        const stickerPackId = packId || (0, crypto_1.randomBytes)(16).toString('hex').replace(/(.{8})(.{4})(.{4})(.{4})(.{12})/, '$1-$2-$3-$4-$5');
        ...
        const { thumbnail: jpegThumbnail } = await (0, messages_media_1.generateThumbnail)(coverBuffer, 'image', options);
        const [stickerPackUpload, coverUpload] = await Promise.all([
            messages_media_1.encryptedStream(zipBuffer, 'sticker', { logger: options.logger, opts: options.options }), 
        ...
        const sha256Hex = crypto_2.sha256(coverBuffer).toString('hex');
        ...
        m.stickerPackMessage = {
            ...
            thumbnailHeight: 252, //coverImage.height || 252, 
            thumbnailWidth: 252 // coverImage.width || 252
        } 
