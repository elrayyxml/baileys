...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
       ...
       pollResultSnapshotMessage.contextInfo = {
        ...
        const { stream: coverStream } = await (0, messages_media_1.getStream)(cover);
        const coverBuffer = await (0, messages_media_1.toBuffer)(coverStream);

        const [stickerPackUpload, coverUpload] = await Promise.all([
            (0, messages_media_1.encryptedStream)(zipBuffer, 'sticker', { logger: options.logger, opts: options.options }),
