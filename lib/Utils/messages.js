...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('interactiveMessage' in message && !!message.interactiveMessage) {
        ...
        if (raw.body) {
            ...
            interactiveMessage.body = Types_1.WAProto.Message.InteractiveMessage.Body.fromObject({ text: raw.text || raw.caption });
        }
        if (raw.footer) {
        ...
        if (raw.image) mediaToUpload = { image: raw.image };
        else if (raw.video) mediaToUpload = { video: raw.video };
        else if (raw.document) mediaToUpload = { document: raw.document, fileName: raw.fileName, mimetype: raw.mimetype };
        ...
        if (raw.mentions) interactiveMessage.contextInfo.mentionedJid = raw.mentions;
