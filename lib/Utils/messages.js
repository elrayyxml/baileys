...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('interactiveMessage' in message && !!message.interactiveMessage) {
        ...
        if (raw.text || raw.caption) {
            interactiveMessage.body = { text: raw.text || raw.caption };
        }
        if (raw.footer) {
            interactiveMessage.footer = { text: raw.footer };
        }
        if (raw.title || raw.header) {
        ...
        let mediaToUpload = raw.image ? { image: raw.image } : (raw.video ? { video: raw.video } : (raw.document ? { document: raw.document } : null));
        if (mediaToUpload) {
        ...
        if (raw.contextInfo || raw.mentions) {
