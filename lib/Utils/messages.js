...
const generateWAMessageContent = async (message, options) => {
    ...
    // wrapper nativeFlowMessage
    if ('nativeFlowMessage' in message && !!message.nativeFlowMessage) {
        const interactiveMessage = {
            nativeFlowMessage: Types_1.WAProto.Message.InteractiveMessage.NativeFlowMessage.fromObject(message.nativeFlowMessage)
        };
        if ('text' in message || 'caption' in message) {
            interactiveMessage.body = { text: message.text || message.caption };
        }
        if ('footer' in message && !!message.footer) {
        ...
        let headerData = {
            ...
            hasMediaAttachment: !!(m.productMessage || message.image || message.video || message.document)
        };
        if (m.productMessage) {
            headerData.productMessage = m.productMessage;
        } else if (message.image || message.video || message.document) {
            Object.assign(headerData, m);
        }
        interactiveMessage.header = Types_1.WAProto.Message.InteractiveMessage.Header.fromObject(headerData);
