...
const generateWAMessageContent = async (message, options) => {
    ...
    const tmpZipPath = path.join(os.tmpdir(), `sp_${Date.now()}.zip`);
    fsSync.writeFileSync(tmpZipPath, zipBuffer);

    const stickerPackUpload = await (0, messages_media_1.encryptedStream)(tmpZipPath, 'sticker-pack', {
        logger: options.logger,
        opts: options.options
    });

    fsSync.unlinkSync(tmpZipPath);

    const stickerPackUploadResult = await options.upload(
        fsSync.createReadStream(stickerPackUpload.bodyPath), {
    ...
    fsSync.unlinkSync(stickerPackUpload.bodyPath);

    let thumbnailBuffer = coverBuffer;
    let thumbnailDirectPath, thumbnailSha256, thumbnailEncSha256, imageDataHash;

    try {
        const tmpThumbPath = path.join(os.tmpdir(), `thumb_${Date.now()}.webp`);
        fsSync.writeFileSync(tmpThumbPath, thumbnailBuffer);

        const thumbUpload = await (0, messages_media_1.encryptedStream)(tmpThumbPath, 'thumbnail-sticker-pack', {
            logger: options.logger,
            opts: options.options,
            mediaKey: stickerPackUpload.mediaKey
        });

        fsSync.unlinkSync(tmpThumbPath);

        const thumbUploadResult = await options.upload(
            fsSync.createReadStream(thumbUpload.bodyPath), {
        ...
        fsSync.unlinkSync(thumbUpload.bodyPath);
