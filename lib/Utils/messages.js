...
const generateWAMessageContent = async (message, options) => {
    ...
    var _p, _q;
    let m = {};
    // Wrapper interactiveMessage
    if ('interactiveMessage' in message && !!message.interactiveMessage) {
        const raw = message.interactiveMessage;
        const interactiveMessage = {};
        if (raw.nativeFlowMessage) {
            interactiveMessage.nativeFlowMessage = Types_1.WAProto.Message.InteractiveMessage.NativeFlowMessage.fromObject(raw.nativeFlowMessage);
        } else if (raw.buttons) {
            interactiveMessage.nativeFlowMessage = Types_1.WAProto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                buttons: raw.buttons,
                messageParamsJson: raw.messageParamsJson || ''
            });
        }
        if (raw.carouselMessage) {
            interactiveMessage.carouselMessage = Types_1.WAProto.Message.InteractiveMessage.CarouselMessage.fromObject(raw.carouselMessage);
        }
        if (raw.body) {
            interactiveMessage.body = Types_1.WAProto.Message.InteractiveMessage.Body.fromObject(raw.body);
        } else if (raw.text || raw.caption) {
            interactiveMessage.body = Types_1.WAProto.Message.InteractiveMessage.Body.fromObject({
                text: raw.text || raw.caption
            });
        }
        if (raw.footer) {
            interactiveMessage.footer = Types_1.WAProto.Message.InteractiveMessage.Footer.fromObject(typeof raw.footer === 'string' ? { text: raw.footer } : raw.footer);
        }
        let headerContent = raw.header || {};
        if (raw.title) headerContent.title = raw.title;
        if (raw.subtitle) headerContent.subtitle = raw.subtitle;
        let mediaToUpload = null;
        if (raw.thumbnail) mediaToUpload = { image: raw.thumbnail };
        else if (raw.image) mediaToUpload = { image: raw.image };
        else if (raw.video) mediaToUpload = { video: raw.video };
        ...
        if (mediaToUpload) {
            const preparedMedia = await (0, exports.prepareWAMessageMedia)(mediaToUpload, options);
            Object.assign(headerContent, preparedMedia);
            headerContent.hasMediaAttachment = true;
        }
        if (Object.keys(headerContent).length > 0) {
            interactiveMessage.header = Types_1.WAProto.Message.InteractiveMessage.Header.fromObject(headerContent);
        }
        interactiveMessage.contextInfo = raw.contextInfo || {};
        if (raw.mentions) {
            interactiveMessage.contextInfo.mentionedJid = raw.mentions;
        }
        if (raw.externalAdReply) {
            interactiveMessage.contextInfo.externalAdReply = raw.externalAdReply;
        }
        m = {
            viewOnceMessage: {
                message: {
                    ...
                    interactiveMessage: Types_1.WAProto.Message.InteractiveMessage.fromObject(interactiveMessage)
                }
            }
        };
    }
